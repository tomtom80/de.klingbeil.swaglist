...
public class WishEventHandler {
  ...
  public void on(WishNameAddedEvent event) {
    wishRepository.save(new WishView(event.getWishId().getIdentifier(),
        event.getWishlistId().getIdentifier(), event.getWishName(), null, null, null, null));
  }

  @EventHandler
  public void on(WishUrlAddedEvent event) {
    wishRepository
        .save(new WishView(event.getWishId().getIdentifier(), event.getWishlistId().getIdentifier(),
  ...
  public void on(WishUpdatedEvent event) {
    ...
    wishRepository.save(new WishView(event.getWishId().getIdentifier(),
        event.getWishlistId().getIdentifier(), event.getWishName(), event.getWishDescrition(),
        event.getWishLocation(), wishLocationUrl != null ? wishLocationUrl.toString() : null,
