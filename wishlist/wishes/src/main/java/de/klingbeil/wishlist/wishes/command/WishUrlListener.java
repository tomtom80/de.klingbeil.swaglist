...
public class WishUrlListener {
	...
	public void on(WishUrlAddedEvent event) {
		logger.debug("About to dispatch a new command to crawle a wish for the url {}", event.getWishUrl());
		commandGateway.send(
				new CrawleUrlCommand(new CrawlerId(), event.getWishlistId(), event.getWishId(), event.getWishUrl()));
	}
}
