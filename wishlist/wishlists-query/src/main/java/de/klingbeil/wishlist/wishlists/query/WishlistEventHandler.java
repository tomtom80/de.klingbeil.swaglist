...
public class WishlistEventHandler {
	...
	public void on(WishlistCreatedEvent event) {
		wishlistRepository.save(new WishlistView(event.getWishlistId().getIdentifier(), event.getWishlistName(),
				event.getWishlistType()));
	}
	...
	public void on(WishlistDeletedEvent event) {
		WishlistView wishlistListView = wishlistRepository.getOne(event.getWishlistId().getIdentifier());
